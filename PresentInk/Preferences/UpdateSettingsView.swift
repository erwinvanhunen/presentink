...
class UpdateSettingsView: NSView {
    ...
    private let checkOnStartupLabel = NSTextField(
    ...
    private let lastCheckedLabel = NSTextField(
        labelWithString: NSLocalizedString("Last checked: Today",comment:"")
    )
    ...
    private func setupUI() {
        ...
        let checkboxStack = NSStackView(views: [
            checkOnStartupLabel, checkOnStartupSwitch,
        ])
    ...
    private func loadSettings() {
        checkOnStartupSwitch.state =
            Settings.shared.checkForUpdatesOnStartup ? .on : .off
    ...
    private func updateLastCheckedLabel() {
        if let lastChecked = Settings.shared.lastUpdateCheck {
            ...
            lastCheckedLabel.stringValue =
                "\(NSLocalizedString("Last checked", comment:"")): \(formatter.string(from: lastChecked))"
        } else {
            lastCheckedLabel.stringValue = NSLocalizedString("Last checked: Never", comment:"")
        }
    }

    @objc private func switchChanged() {
        Settings.shared.checkForUpdatesOnStartup =
            checkOnStartupSwitch.state == .on
    }

    @objc private func checkNowPressed() {
        checkNowButton.isEnabled = false
        checkNowButton.title = NSLocalizedString("Checking...",comment:"")

        UpdateChecker.checkForUpdates { [weak self] result in
            DispatchQueue.main.async {
                self?.checkNowButton.isEnabled = true
                self?.checkNowButton.title = NSLocalizedString("Check Now", comment:"")
    ...
    private func showUpdateAvailableAlert(_ updateInfo: UpdateInfo) {
        ...
        alert.informativeText =
            "\(NSLocalizedString("A new version",comment:"")) (\(updateInfo.latestVersion)) \(NSLocalizedString("is available. You are currently using version",comment:"")) \(updateInfo.currentVersion)."
    ...
    private func showNoUpdateAlert() {
        let alert = NSAlert()
        alert.messageText = NSLocalizedString("No Updates Available",comment:"")
        alert.informativeText =
        NSLocalizedString("You are running the latest version of PresentInk.",comment:"")
        alert.alertStyle = .informational
    ...
    private func showErrorAlert(_ error: Error) {
        ...
        alert.informativeText =
        "\(NSLocalizedString("Unable to check for updates:",comment:"")) \(error.localizedDescription)"
